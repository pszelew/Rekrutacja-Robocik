...
class Flatten(Layer):
    ...
    def back(self, prop: np.array, prev_output: np.array, next_weights: np.array, lr: np.float64):
        ...
        loss_derivative = np.zeros(self.output_dims[1])
        ...
        for i in range(len(loss_derivative)):
            ...
            for j in range(len(prop)):
                ...
                loss_derivative[i] += next_weights[j][i] * prop[j]
        ...
        prop = np.reshape(loss_derivative, (prev_output.shape[1], prev_output.shape[2], -1))
        print(prop.shape)
        return prop